...
char* detect_virus(char *buffer, link *virus_list, unsigned int size){
  int i;
  int j;
  int h;
  char* temp;
  ...
  for(i = 0 ; i < list_size ; i ++){
	...
	for(h = j; h<j + virus_list->v->length; h++)
	  buffer[h]=0x90;
      }
      temp = temp + 1;
    }
...
int main(int argc, char **argv) {
    ...
    while(size>0){
      first = fgetc(fp);
      second = fgetc(fp);
      v =  (struct virus*)(malloc((first*10 + second)*sizeof(char)));
      v->length = first*10 + second - 18;
    ...
    outputSize = ftell (fpRead);
    rewind (fpRead);
    if(outputSize > 10*1024)
      outputSize = 10*1024;
    printf("\n--------------------\nDetecting viruses in %s!\n\n",argv[1]);
    sizeRead = fread(toCheck, sizeof(char), outputSize, fpRead);
    fwrite(detect_virus(toCheck, virus_list, sizeRead),sizeof(char), outputSize,  fpRead);
    fclose(fpRead);
